...
export async function getFilingItemTypes(): Promise<FilingItemTypes> {
  const baseUrl = process.env.FREE_DATA_API_URL || 'https://api.financialdatasets.ai';
  const response = await fetch(`${baseUrl}/filings/items/types/`);
  if (!response.ok) {
    // Fallback: return static types if endpoint unavailable (e.g., local server)
    return {
      '10-K': [
        { name: 'Item-1', title: 'Business' },
        { name: 'Item-1A', title: 'Risk Factors' },
        { name: 'Item-7', title: "Management's Discussion and Analysis" },
        { name: 'Item-8', title: 'Financial Statements' },
      ],
      '10-Q': [
        { name: 'Part-1,Item-1', title: 'Financial Statements' },
        { name: 'Part-1,Item-2', title: "Management's Discussion and Analysis" },
        { name: 'Part-1,Item-3', title: 'Market Risk' },
