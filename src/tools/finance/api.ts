...
const BASE_URL = process.env.FREE_DATA_API_URL || 'https://api.financialdatasets.ai';
...
export async function callApi(
  ...
  const usingLocalServer = Boolean(process.env.FREE_DATA_API_URL);

  if (!FINANCIAL_DATASETS_API_KEY && !usingLocalServer) {
  ...
  // Build headers — skip auth when using local free data server
  const headers: Record<string, string> = {};
  if (!usingLocalServer && FINANCIAL_DATASETS_API_KEY) {
