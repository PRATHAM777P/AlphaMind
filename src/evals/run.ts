/**
 * LangSmith Evaluation Runner for AlphaMind
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps AlphaMind agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `alphamind-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'alphamind-finance-eval';
    ...
    const experimentName = `alphamind-eval-${Date.now().toString(36)}`;
